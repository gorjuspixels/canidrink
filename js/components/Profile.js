...
import { createProfile, changeTitle } from '../actions/HomeActions';
...
class Profile extends Component {
  ...
  renderProfile = () => {
    const { name, weight, gender, bacLimit } = this.props.profile
    ...
    return (
      <div style={ styles.flexContainer }>
        <div>
          ...
          <TextField ref="hours" hintText="Amount of hours you've been drinking for" />
        </div>
        <div style={{ paddingTop: '10px' }}>
          <div style={{
            display: 'inline-block',
            verticalAlign: 'middle',
            color: Colors[this.getBCAStyle(currentBAC)]
          }}>{ currentBAC + '%' }</div>
          <IconButton
  ...
  getBCAStyle(bac) {
    if (!bac || bac < 0.5) return 'green500'
    if (bac < 0.8) return 'yellow500'
    return 'red500'
  }
  ...
  calculateBAC() {
    ...
    return (numStandard / (weight * GENDER_CONSTANT[genders[gender]])) * 100 - (hours * 0.015)
  }
